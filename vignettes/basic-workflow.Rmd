...
*Note 7/26/2021:* The Census Bureau's QWI API is down, so these next couple chunks aren't running right now, but the code is still accurate :-/

```{r qwi1, eval=FALSE, include=TRUE}
...
```{r qwi2, eval=FALSE, include=TRUE}
top2018 <- employment %>%
...
```{r inflation, eval=FALSE, include=TRUE}
employment %>%
  filter(industry %in% top2018) %>%
  mutate(label = stringr::str_sub(label, 1, 25)) %>%
