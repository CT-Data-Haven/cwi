...
* Call `add_grps()` with a list of racial groups and their labels' positions in the `label` column. This gives estimates and, optionally, margins of error for aggregates
...
gnh_data$race |>
    ...
    group_by(level, name) |>
    add_grps(
        list(total = 1, white = 3, black = 4, latino = 12, other = 5:9),
        group = label
    ) |>
...
tenure_map <- homeownership |>
    ...
    quick_map(
        ...
        color = "black",
        linewidth = 0.4,
        title = "Homeownership, Greater New Haven",
        palette = "BuPu"
    )
...
age |>
    ...
    group_by(level, name) |>
    add_grps(
        list(ages00_04 = 1, ages05_17 = 2:4, ages00_17 = 1:4),
        group = age,
        value = estimate
    ) |>
...
employment |>
    ...
    labs(
        ...
        subtitle = "Connecticut and South Central COG, 2002-2018",
        color = "Industry"
    ) +
    theme_minimal() +
...
unemp_plot <- ggplot(
    unemployment,
    aes(x = date, y = value, group = area, color = area)
) +
    geom_line() +
