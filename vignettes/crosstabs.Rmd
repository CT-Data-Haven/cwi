...
![Screenshot of DCWS crosstabs spreadsheet](../man/figures/xtab-screenshot.png "Screenshot of DCWS crosstabs spreadsheet")
...
```{r chart, fig.width=6, fig.height=3}
library(ggplot2)
library(showtext)
...
cws %>%
  ...
  geom_text(aes(label = lbl), hjust = 1, nudge_y = -0.01, fontface = "bold", family = "Barlow Semi Condensed", color = "white") +
...
```{r govt_chart, fig.width=6, fig.height=4}
govt <- cws %>%
...
cws_out %>%
  ...
  labs(x = NULL, y = NULL,
       title = "Neighborhood satisfaction is high, even when government feels unresponsive",
