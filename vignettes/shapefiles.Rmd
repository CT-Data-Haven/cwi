...
neighborhood_map <- function(shp, city_name) {
  ...
  bbox <- sf::st_bbox(shp) %>% stats::setNames(c("left", "bottom", "right", "top"))
  ...
  ggmap::ggmap(basemap, darken = c(0.4, "white")) +
    geom_sf(data = shp, inherit.aes = FALSE, color = "mediumorchid3", fill = "orchid3", alpha = 0.1) +
    geom_sf(data = city_shp, inherit.aes = FALSE, color = "#7636C7", fill = NA, size = 0.8) +
    geom_sf_text(aes(label = name), data = shp, inherit.aes = FALSE, size = 2.8) +
    theme_void()
}
```
