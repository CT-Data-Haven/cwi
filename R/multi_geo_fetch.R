...
#' @param year The year of the ACS table; currently defaults 2022 (most recent available).
...
#' }
#' @export
multi_geo_acs <- function(table, year = 2022, towns = "all", regions = NULL,
...
#' @param year The year of the census table; currently defaults 2020.
...
multi_geo_decennial <- function(table, year = 2020, towns = "all", regions = NULL,
  ...
  ## PARAMS & ERROR HANDLING ----
  params <- multi_geo_prep(src = "decennial",
                           table = table,
                           year = year,
                           towns = towns,
                           regions = regions,
...
multi_geo_prep <- function(src,
  ...
  if (is.logical(dataset_title) && !dataset_title) {
    cli::cli_abort(c("Dataset {dataset} for year {year} is not available in the API.",
