...
lookup <- readr::read_delim(
    ...
    dplyr::select(
        town = NAMELSAD_COUSUB_22,
        zip = GEOID_ZCTA5_20,
        inter_area = AREALAND_PART
    ) |>
    dplyr::filter(
...
block_town <- block_pop |>
    sf::st_drop_geometry() |>
    dplyr::inner_join(
        dplyr::distinct(cwi::xwalk, block, town),
        by = c("geoid" = "block")
    ) |>
...
zip2town <- block_dfs |>
    ...
    tidyr::pivot_wider(
        id_cols = town_name:zip_name,
        names_from = group,
        values_from = c(dplyr::matches("(inter|pct)"))
    ) |>
