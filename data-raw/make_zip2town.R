...
lookup <- readr::read_delim("https://www2.census.gov/geo/docs/maps-data/data/rel2022/acs22_cousub22_zcta520_st09.txt",
  ...
  dplyr::filter(!grepl("County subdivisions", town),
                !is.na(zip)) |>
...
zip_sf <- tigris::zctas(cb = FALSE, starts_with = "06", year = 2022) |>
  ...
  sf::st_transform(2234)
block_pop <- tidycensus::get_decennial("block", variables = c(pop = "P1_001N", hh = "H4_001N"),
...
zip2town <- block_dfs |>
  ...
  dplyr::mutate(pct_of_town =  round(inter / town, digits = 4),
