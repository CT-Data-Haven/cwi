# WRITE: occ_codes, naics_codes

# INDUSTRY CODES -- NAICS
naics_codes <- readr::read_csv(
...
qwi_avail <- rvest::read_html(
    ...
    dplyr::mutate(dplyr::across(
        ...
        ~ as.numeric(stringr::str_extract(., "\\d{4}"))
    )) |>
    dplyr::inner_join(
        dplyr::distinct(tidycensus::fips_codes, state, state_code),
        by = "state"
    ) |>
    dplyr::select(
        state_code,
...
if (!file.exists(occ_fn)) {
    download.file(
        url = "https://www2.census.gov/programs-surveys/demo/guidance/industry-occupation/2018-ACS-PUMS-and-2018-SIPP-Public-Use-Occupation-Code-List.xlsx",
        destfile = occ_fn
    )
}
top_occ <- readxl::read_excel(
    occ_fn,
    sheet = 1,
    range = "C4:C8",
    ...
    tidyr::extract(
        description,
        into = c("occ_code", "description"),
        regex = "(\\d{4}\\s\\-\\s\\d{4})\\s+([\\w\\s[:punct:]]+)$"
    ) |>
...
occ <- readxl::read_excel(
    occ_fn,
    sheet = 1,
    skip = 10,
    col_names = c("occ_code", "soc_code", "description")
) |>
    dplyr::filter(
        !grepl("Combines", soc_code),
        !is.na(soc_code),
        !is.na(occ_code)
    ) |>
...
occ_codes <- occ |>
    ...
    dplyr::filter(is_hdr) |>
    dplyr::mutate(
        is_major_grp = tidyr::replace_na(
            (is_top | description %in% occ_grps),
            FALSE
        )
    ) |>
    dplyr::mutate(
        occ_group = ifelse(is_major_grp, description, NA_character_)
    ) |>
    ...
    ) # this is really broad--want to separate these out
