...
town_puma <- list(
    county = cwi::xwalk |>
        ...
        dplyr::rename(puma = puma_fips),
    cog = cwi::xwalk |>
        dplyr::distinct(town, puma_fips_cog) |>
        dplyr::rename(puma = puma_fips_cog)
) |>
...
reg_df <- cwi::regions[c(
    "Greater New Haven",
    "Greater Hartford",
    "Greater Waterbury",
    "Lower Naugatuck Valley",
    "Greater Bridgeport",
    names(cwi::regions)[grep("COG$", names(cwi::regions))]
)] |>
...
pops <- tidyr::expand_grid(
    ...
    dplyr::mutate(
        data = purrr::pmap(
            list(puma_type, year, geo),
            function(puma_type, year, geo) {
                tidycensus::get_acs(
                    geo,
                    variables = c(pop = "B01003_001", hh = "B25003_001"),
                    state = "09",
                    year = year
                )
            }
    ...
    dplyr::mutate(
        data = purrr::modify_at(data, geo == "puma", \(x) {
            dplyr::select(x, -name) |> dplyr::rename(name = geoid)
        })
    ) |>
    dplyr::mutate(
        data = purrr::map(
    ...
    dplyr::mutate(
        geo = forcats::fct_recode(geo, town = "county subdivision")
    ) |>
    tidyr::unnest(data) |>
    split(~geo) |>
    purrr::map(
        tidyr::pivot_wider,
        id_cols = c(puma_type, name, year),
        names_from = geo,
        values_from = pop:hh
    )
