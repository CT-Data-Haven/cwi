...
decennial_vars <- clean_decennial_vars(
...
include_str <- c(
    "Detailed Tables",
    "PL 94-171",
    "Summary File \\d(?! Demographic Profile)",
    "SF\\d",
...
cb_avail <- purrr::keep(cb_avail, \(x) {
    grepl(include_patt, x$title, perl = TRUE) &
        !grepl(exclude_patt, x$title, perl = TRUE)
})
cb_avail <- purrr::map(cb_avail, purrr::modify_at, "c_dataset", \(x) {
    rlang::set_names(x, c("program", "survey"))
})
cb_avail <- purrr::map_dfr(cb_avail, \(x) {
    tibble::tibble(
        vintage = x$c_vintage,
...
cb_avail$title <- stringr::str_remove_all(
    cb_avail$title,
    "(Census: |Estimates: | \\d\\-Year$)"
)
cb_avail$title <- stringr::str_replace_all(
    cb_avail$title,
    c("Summary File " = "SF", "American Community Survey:" = "ACS")
)
