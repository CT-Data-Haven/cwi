test_that("qwi_industry handles county defaults", {
    ...
    expect_equal(
        length(unique(all_county$county)),
        length(unique(cwi::xwalk$cog))
    )
})
...
test_that("qwi_industry handles errors, including from COG mismatches", {
    ...
    expect_error(
        qwi_industry(2024, state = "09", counties = "009"),
        "replaced counties with COGs"
    )
...
test_that("qwi_industry handles years not in API", {
    ...
    expect_equal(
        nrow(suppressWarnings(qwi_industry(
            1991:2000,
            industries = "23",
            annual = TRUE
        ))),
...
test_that("qwi_industry handles long time periods", {
    skip_on_ci()
    expect_message(
        qwi_industry(2000:2016, industries = "23", annual = TRUE),
        "multiple calls"
    )
    expect_equal(
        nrow(qwi_industry(2000:2016, industries = "23", annual = TRUE)),
        17
    )
})
