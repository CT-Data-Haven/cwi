test_that("separate_acs handles column specifications", {
    ...
    expect_equal(
        names(suppressWarnings(separate_acs(df))),
        c("geoid", "x1", "x2", "x3")
    )
    expect_length(suppressWarnings(separate_acs(df)), 4)
    expect_length(suppressWarnings(separate_acs(df, drop_total = TRUE)), 3)
    expect_length(
        suppressWarnings(separate_acs(df, into = c("total", "sex", "age"))),
        4
    )
    expect_length(
        suppressWarnings(separate_acs(
            df,
            into = c("sex", "age"),
            drop_total = TRUE
        )),
...
test_that("separate_acs passes arguments to tidyr::separate", {
    ...
    expect_true(any(is.na(
        suppressWarnings(separate_acs(df, fill = "left"))$x1
    )))
})
