test_that("bls functions check for API key", {
    ...
    expect_gt(
        nchar(bls_test(year = 2011:2014)$query[[1]]$body$registrationKey),
        0
    )
})
...
test_that("adj_inflation correctly calculates adjusted amounts", {
    ...
    df2000 <- adj_inflation(
        data.frame(year = 2000, amount = 100),
        value = amount,
        year = year,
        base_year = 2016
    )
...
test_that("get_cpi matches series ID with seasonality", {
    ...
    yrs <- 2010:2014
    base <- 2015
    seas <- bls_test(seasonal = TRUE, year = yrs)[["query"]][[1]][["body"]][[
        "seriesid"
    ]]
    unseas <- bls_test(seasonal = FALSE, year = yrs)[["query"]][[1]][["body"]][[
        "seriesid"
    ]]
