test_that("get_county_fips matches & returns FIPS codes", {
    ...
    expect_equal(
        get_county_fips("09", c("New Haven County", "Fairfield County"), FALSE),
        correct
    )
    expect_equal(
        get_county_fips("09", c("New Haven", "Fairfield"), FALSE),
        correct
    )
    ...
    expect_equal(
        get_county_fips("09", c("09009", "09001", "Fairfield"), FALSE),
        correct
    )
    ...
    expect_equal(
        get_county_fips(
            "09",
            c("Western Connecticut COG", "Greater Bridgeport COG"),
            TRUE
        ),
        correct_cog
    )
...
test_that("get_county_fips catches mismatched county names", {
    correct <- c("09001", "09009")
    expect_equal(
        suppressWarnings(get_county_fips(
            "09",
            c("New Haven County", "Fairfield County", "Cook County"),
            FALSE
        )),
        correct
    )
    expect_warning(get_county_fips(
        "09",
        c("New Haven County", "Fairfield County", "Cook County"),
        FALSE
    ))
