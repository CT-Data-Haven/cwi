test_that("geo_printout handles counties vs COGs", {
    ...
    expect_message(
        multi_test(year = 2021, counties = "Fairfield County"),
        "all towns in Fairfield County"
    )
    expect_message(
        multi_test(year = 2022, counties = "Capitol COG"),
        "all towns in Capitol COG"
    )
})
...
test_that("geo_printout removes duplicates", {
    ...
    expect_no_message(
        multi_test(regions = regs),
        message = "(Greater Hartford).+(\\1)"
    )
})
