...
test_that("town_names removes 'town, * County'", {
  skip_on_ci()
  pops21 <- tidycensus::get_acs(geography = "county subdivision", variables = "B01003_001",
                              state = "09", year = 2021,
                              key = Sys.getenv("CENSUS_API_KEY"))
  clean21 <- town_names(pops21, NAME)

  pops22 <- tidycensus::get_acs(geography = "county subdivision", variables = "B01003_001",
                                state = "09", year = 2022,
                                key = Sys.getenv("CENSUS_API_KEY"))
